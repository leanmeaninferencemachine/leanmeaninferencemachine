...
from app.tools.scraper_tools import ScraperAgent, ScraperAnalyzer
...
def api_scraper_scrape():
    ...
    mode = data.get("mode", "basic")  # "basic" or "lmim"
    ...
    results = scraper.scrape_sync(urls)
    ...
    if mode == "lmim" and purpose and successful:
        analyzer = ScraperAnalyzer()
        analysis = analyzer.analyze_sync(successful, purpose)
        if analysis:
...
def api_scraper_analyze():
    """Run AI analysis on previously scraped content."""
    data = request.json or {}
    results = data.get("results", [])
    purpose = data.get("purpose", "")

    if not results or not purpose:
        return jsonify({"error": "Missing results or purpose"}), 400

    analyzer = ScraperAnalyzer()
    analysis = analyzer.analyze_sync(results, purpose)
...
from app.tools.contacts_tools import list_contacts_api, add_contact, delete_contact

@app.route("/api/contacts/list", methods=["GET"])
def api_contacts_list():
    user_id = request.args.get('user_id', 'default')
    return jsonify(list_contacts_api(user_id))
...
def api_contacts_add():
    ...
    result = add_contact(user_id, name, phone, email, notes)
    if result.get("success"):
        return jsonify({"success": True, "contact": result.get("contact")})
...
def api_contacts_delete():
    ...
    result = delete_contact(user_id, contact_id)
    if result.get("success"):
...
def get_config_status():
    ...
    response_data = {
        ...
        "prime_directive": get_val("PRIME_DIRECTIVE"),
        ...
        with open(env_path, "r+", encoding="utf-8") as f:
            ...
            for form_key, env_key in mapping.items():
                if form_key in data and data[form_key] is not None:
                    ...
                    if form_key == "prime_directive":
                        current_env["PRIME_DIRECTIVE"] = new_val
                        changes_made += 1
                    if form_key == "language":
                        current_env["LANGUAGE"] = new_val
                        changes_made += 1
...
from app.rag import store_document, clear_document, get_document_info, extract_text_from_file

@app.route("/api/rag/upload", methods=["POST"])
def api_rag_upload():
    """Upload a document for RAG: PDF, TXT, or MD."""
    if 'file' not in request.files:
        return jsonify({"error": "No file provided"}), 400
    
    file = request.files['file']
    user_id = request.form.get('user_id', request.args.get('user_id', 'default'))
    ...
    try:
        ...
        result = store_document(user_id, text, filename)
        if "error" in result:
            return jsonify({"error": result["error"]}), 500
        
        return jsonify({"ok": True, "chunks": result["chunks"], "filename": result["filename"]})
    except Exception as e:
...
def api_rag_clear():
    """Clear the active RAG document for a user."""
    data = request.json or {}
    user_id = data.get('user_id', 'default')
...
def api_rag_info():
    """Get info about the active RAG document."""
    user_id = request.args.get('user_id', 'default')
    info = get_document_info(user_id)
    if info:
        return jsonify({"active": True, **info})
...
def get_daemon_status():
    ...
    status = {
        "whatsapp": {"status": "offline", "queue": 0, "last_activity": "N/A", "phone": None},
    ...
    # ── WHATSAPP: Query Baileys IPC directly (not heartbeat) ────────────────
    try:
        wa_resp = requests.get(f'{_WA_IPC_BASE}/status', timeout=2)
        if wa_resp.status_code == 200:
            wa_data = wa_resp.json()
            if wa_data.get('loggedIn') and wa_data.get('connected'):
                status["whatsapp"]["status"] = "online"
                status["whatsapp"]["last_activity"] = datetime.now().strftime('%H:%M:%S')
                status["whatsapp"]["phone"] = wa_data.get('phone')
            elif wa_data.get('loggedIn') == False and wa_data.get('hasQR'):
                status["whatsapp"]["status"] = "qr_ready"
                status["whatsapp"]["last_activity"] = "Scan QR code"
            elif wa_data.get('loggedIn') == False:
                status["whatsapp"]["status"] = "waiting"
                status["whatsapp"]["last_activity"] = "Starting..."
            else:
                status["whatsapp"]["status"] = "connecting"
        else:
            status["whatsapp"]["status"] = "offline"
    except requests.exceptions.ConnectionError:
        status["whatsapp"]["status"] = "offline"
        logger.debug("Baileys IPC not reachable")
    except Exception as e:
        logger.debug(f"Baileys IPC status check failed: {e}")
        status["whatsapp"]["status"] = "error"

    # ── OTHER DAEMONS: Heartbeat file check ─────────────────────────────────
    daemons = ["telegram", "email", "slack", "discord"]
    ...
    # ── WhatsApp Queue Specifics (still useful for tracking) ────────────────
...
def is_whatsapp_session_valid():
    """
    Check if WhatsApp session is valid by querying Baileys IPC directly.
    Returns True only if daemon is running AND user is logged in.
    """
    # ── Primary: Query Baileys IPC ──────────────────────────────────────────
    try:
        resp = requests.get(f'{_WA_IPC_BASE}/status', timeout=2)
        if resp.status_code == 200:
            data = resp.json()
            # Session valid ONLY if logged in AND connected
            logged_in = bool(data.get('loggedIn', False))
            connected = bool(data.get('connected', False))
            return logged_in and connected
    except requests.exceptions.ConnectionError:
        # IPC server not reachable — daemon not running
        pass
...
# Also update the /api/internal/wa-qr endpoint to return status when connected
@app.route("/api/internal/wa-qr", methods=["GET"])
def get_wa_qr():
    """Returns the WhatsApp QR code from Baileys for pairing, or connection status if already connected."""
    
    # FIRST: Check if already connected via Baileys IPC
    try:
        status_resp = requests.get(f'{_WA_IPC_BASE}/status', timeout=2)
        if status_resp.status_code == 200:
            status = status_resp.json()
            if status.get('loggedIn') and status.get('connected'):
                return jsonify({
                    "ok": True,
                    "loggedIn": True,
                    "connected": True,
            ...
            # Also return hasQR status so frontend knows QR is available
            if status.get('hasQR'):
                # Try to get the actual QR code
                try:
                    qr_resp = requests.get(f'{_WA_IPC_BASE}/qr', timeout=2)
...
from app.workspace import set_workspace, get_workspace, clear_workspace
...
def api_set_workspace():
    ...
    result = set_workspace(data["path"])
    if "error" in result:
...
def api_workspace_ls():
    rel_path = request.args.get("path", ".")
    result = list_files(rel_path)
    if not result.get("success"):
        return jsonify({"error": result.get("error")}), 400
    return jsonify({"result": result.get("files")})
...
def delete_chat():
    ...
    import shutil
    data = request.json or {}
    # Accept both 'chat_id' and 'user_id' for compatibility with frontend
...
def _system_env() -> dict:
    """
    Return an env dict safe for calling system binaries (ffmpeg, etc.) from
    inside an AppImage.  Strips the AppImage's bundled lib dirs from
    LD_LIBRARY_PATH so system tools use the host's libstdc++/libssl instead
    of the Ubuntu-built versions we ship (which lack newer GLIBCXX symbols
    required by Fedora/Arch system codecs like libsrt, libvvenc, libtesseract).
    """
    env = os.environ.copy()
    meipass = getattr(sys, '_MEIPASS', '')
    if meipass:
        ldp = env.get('LD_LIBRARY_PATH', '')
        clean = [p for p in ldp.split(':')
...
def voice_transcribe():
    ...
    try:
        ...
        logger.error('❌ Whisper timed out after 120s')
        return jsonify({'ok': False,
                        'error': 'Transcription timed out (>120s). '
                                 'Download ggml-tiny.bin for much faster inference.'}), 504
    except Exception as e:
        import traceback
...
def voice_speak():
    ...
    language should be ISO 639-1 code: 'es', 'en', 'pt', etc.
    Returns: audio/wav stream.
    """
    try:
        # Import the robust logic from voice_service
        from app.voice_service import tts_synthesize
        ...
        # Use the service function which handles mapping and fallbacks correctly
        wav_bytes = tts_synthesize(text, language=lang)
        ...
        logger.error(f"Voice file missing: {e}")
...
# ==============================================================================
# ==============================================================================
# 🔊 PIPER WARMUP & SHUTDOWN (Singleton process manager)
# ==============================================================================

def warmup_piper_async():
    """Start Piper warmup in background thread to avoid blocking startup."""
    import threading
    def _warmup():
        try:
            from app.voice_service import warmup_piper
            warmup_piper()
            logger.info("🔊 Piper TTS warmed up successfully")
        except Exception as e:
...
def shutdown_piper():
    """Clean shutdown of Piper process."""
    try:
        from app.voice_service import shutdown_piper
        shutdown_piper()
        logger.info("🔊 Piper TTS shut down")
    except Exception as e:
        logger.debug(f"Piper shutdown error: {e}")
...
def api_model_download():
    ...
    model_name = data.get("name", "downloaded_model.gguf")
    
    if not model_url:
        # Default: Qwen 3.5 9B Q5_K_M from Hugging Face
        model_url = "https://huggingface.co/Qwen/Qwen3.5-9B-GGUF/resolve/main/qwen3.5-9b-q5_k_m.gguf"
        model_name = "qwen3.5-9b-q5_k_m.gguf"
    ...
    def download_with_progress():
        try:
            response = dl_requests.get(model_url, stream=True, timeout=30)
            ...
            # Update available models list for UI
            update_available_models_cache()
    ...
    return jsonify({
        "success": True,
        "message": f"Downloading {model_name} in background",
        "dest_path": str(dest_path)
    })

@app.route("/api/models/download-progress", methods=["GET"])
def api_model_download_progress():
    """Get current download progress for active model download."""
    # Simplified: check if a model is currently being downloaded
    # For production, would need to track active downloads per session
    return jsonify({"active": False, "progress": 0})

# ==============================================================================
# RAG DEDICATED CHAT ENDPOINT (no tools, no system prompt)
# ==============================================================================
@app.route("/api/rag/chat", methods=["POST"])
def api_rag_chat():
    """
    Focused document Q&A — bypasses query_model, no system prompt, no tools.
    Handles both content and reasoning_content (Qwen3 thinking model).
    """
    from app.rag import has_document, retrieve_chunks
    from app.config import LOCAL_INFERENCE_URL, MODEL_NAME
    ...
    # Resolve which user_id has an active document
    for uid in [user_id, "admin_user", "default"]:
        if has_document(uid):
            user_id = uid
            break

    if not has_document(user_id):
        return jsonify({"answer": "No document loaded. Please upload a document first."})

    # ── Retrieve relevant chunks ──────────────────────────────────────────────
    def _retrieve(top_k, min_sim):
        try:
            return retrieve_chunks(user_id, question, top_k=top_k, min_similarity=min_sim)
        except TypeError:
            return retrieve_chunks(user_id, question, top_k=top_k)

    chunks = _retrieve(top_k=5, min_sim=0.12)
    if not chunks:
        chunks = _retrieve(top_k=8, min_sim=0.05)

    if not chunks:
        return jsonify({"answer": "I could not find relevant content in the document for that question."})
    ...
    # ── Call llama-server directly with reasoning disabled ────────────────────
    try:
        logger.info(f"RAG request to {LOCAL_INFERENCE_URL}, question: {question[:60]}")
        resp = _req.post(
            LOCAL_INFERENCE_URL,
            json={
                "model":       MODEL_NAME,
                ...
                "max_tokens":  512,
                "stop":        ["</s>"],
                "reasoning":   False,          # <-- explicit disable
            },
            timeout=120,
        )
        ...
        # Qwen3 thinking model: answer is in reasoning_content
        answer = msg.get("content") or msg.get("reasoning_content") or ""
        answer = answer.strip()

        if not answer:
            logger.warning("RAG: both content and reasoning_content are empty")
            answer = "The model returned an empty response. The document may not contain enough context for this question."

        logger.info(f"RAG answer length: {len(answer)} chars")
        return jsonify({"answer": answer})

    except _req.exceptions.ConnectionError:
        logger.error("RAG: cannot connect to llama-server")
        return jsonify({"answer": "⚠️ Local model is not running. Please wait for it to start and try again."})
    except _req.exceptions.Timeout:
        logger.error("RAG: llama-server timeout")
        return jsonify({"answer": "⚠️ The model took too long to respond. Try a shorter question or wait a moment."})
    except Exception as e:
